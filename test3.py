...
def search(event=None):
    ...
    for product_n in naver_product_names:
        ...
        words_n = [re.sub(r'[^a-zA-Z가-힣]', '', word_n).lower() for word_n in words_n if
                   not re.match(r'[a-zA-Z0-9\W]+', word_n)]
        # 결과 리스트에 추가
    ...
    # 빈도수가 1 이상인 단어들만 추출
    filtered_words_naver = {word: count for word, count in word_counts_n.items() if count > 1}

    # 빈도수에 따라 정렬
    sorted_words_n = sorted(filtered_words_naver.items(), key=lambda x: x[1], reverse=True)
    ...
    # 단어 빈도수 계산
    word_count_g = Counter(combined_string.split())

    # 빈도수가 1 이상인 단어들만 추출
    filtered_words = {word: count for word, count in word_count_g.items() if count > 1}
    ...
    for product_element_11 in st11_results:
        ...
        words_11st = product_name_11.split()
        all_words_11st.extend(words_11st)

    # 각 단어의 빈도수 계산
    word_count_11st = Counter(all_words_11st)

    # 빈도수가 1 이상인 단어들만 추출
    filtered_words_11str = {word: count for word, count in word_count_11st.items() if count > 1}

    # 빈도수에 따라 내림차순으로 정렬
    sorted_words__11st = sorted(filtered_words_11str.items(), key=lambda x: x[1], reverse=True)

    # 빈도수에 따라 결과 입력
    for word_11st, count_11st in sorted_words__11st:
        if count_11st > 0:
            st11_product_results.append((word_11st, count_11st))
    ...
    # 11번가 검색 결과 표시
    result_text_11st.insert(tk.END, "\n11번가 검색 결과:\n")
    for product_name, count in st11_product_results:
        product_name = product_name.strip("(),'")
...
result_text_11st.grid(row=1, column=3, padx=5, pady=5)
